...
function vecScale(a, b)
	return {x=a.x*b/vecDist(a),y=a.y*b/vecDist(a)}
end
...
function onTick()
	...
	while not done do
		...
		for i = 1, #obstacles do
			...
			if det == 0 then
				t = -B / (2 * A)
				if vecDist(vecSub(minVec, { x = minVec.x + t * dx, y = minVec.y + t * dy })) < 20 then
					--print("Fuck you")
					goto skip
				end
			elseif det > 0 then
...
function onDraw()
	...
	for i = 0, math.pi*2, math.pi/10 do
	    ...
	    if lp then
	        screen.drawLine(c.x+p.x, c.y+p.y, c.x+lp.x, c.y+lp.y)
	    end
	    lp = p
	end
end
