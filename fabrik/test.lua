...
function drawCircle(x, y, radius, fromAngle, toAngle)
    local fromAngle = fromAngle or 0
    local toAngle = toAngle or tau
...
function onTick()
       ...
       while diff > 0.01 and iterations <= 10 do
            ...
            while node.parent do
                ...
                if node.id > 2 then
                    ...
                    p1 = { x = node.parent.x + node.length * math.cos(angle + node.angleLimit), y = node.parent.y + node.length * math.sin(angle + node.angleLimit) }
                    p2 = { x = node.parent.x + node.length * math.cos(angle - node.angleLimit), y = node.parent.y + node.length * math.sin(angle - node.angleLimit) }
                    local angle2 = angleBetween(node.parent, node.parent.parent, node)
                    --print(angle, angle2)
                    if angle2 < (pi - node.angleLimit) then
                        if dist(p1, node.parent.parent) < dist(p2, node.parent.parent) then
                    ...
                    node.parent.parent.y = math.min(160, node.parent.parent.y)
                    node.parent.y = math.min(160, node.parent.y)
                end
                node = node.parent
            end
            ...
            while node.child do
                ...
                local n = norm(d)
                local s = scale(n, node.length)
                node.child.x = node.x + s.x
                node.child.y = math.min(160, node.y + s.y)
                node = node.child
            end
            diff = dist(node, target)
        end
    ...
    while node.child do
        if node.id > 1 and node.parent then
            output.setNumber(node.id, angle(vecSub(node, node.parent), vecSub(node.child, node))/tau)
        end
        node = node.child
    end
    output.setNumber(1, math.atan((tree.child.x - tree.x), -(tree.child.y - tree.y))/tau)
end
...
function onDraw()
    for b = 1, #obstacles do
        screen.setColor(table.unpack(obstacles[b].color))
        screen.drawRect(obstacles[b].x, obstacles[b].y, obstacles[b].width, obstacles[b].height)
    end
    ...
    drawCircle(144, 160, maxLen, 0, -pi)
    local node = effector
