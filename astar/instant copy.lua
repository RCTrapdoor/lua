...
function check(active_id)
    ...
    if active.x - 1 > 0 and map[active.x - 1][active.y] == 0 then
        ...
        table.insert(possible, {x = active.x - 1, y = active.y, d = dist({x = active.x - 1, y = active.y}, target) + active.v + dist({x = active.x - 1, y = active.y}, start), parent = active_id, v = active.v + 1, done = false})
    end
    ...
    if active.y - 1 > 0 and map[active.x][active.y - 1] == 0 then
        ...
        table.insert(possible, {x = active.x, y = active.y - 1, d = dist({x = active.x, y = active.y - 1}, target) + active.v + dist({x = active.x, y = active.y - 1}, start), parent = active_id, v = active.v + 1, done = false})
    end
...
function onTick()
    possible = {{x = start.x , y = start.y, d = dist({x = start.x, y = start.y}, target), parent = 0, v = 0, done = true}}
    active = 1
    ticks = ticks + 1
    route = {}
    done = false
