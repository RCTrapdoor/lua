...
tree = {id = 1, x = 144, y = 139, length = 20, angleLimit = math.pi / 2}
...
function make_box(x, y, width_, height_, color)
    local box = {
        x = x or math.random(0, width),
        y = y or math.random(0, height),
        width = width_ or math.random(10, 50),
        height = height_ or math.random(10, 50),
        color = color or {math.random(10, 50), math.random(0, 10), math.random(0, 10)}
    }
    return box
end
...
function init_boxes()
    boxes = {}
    while #boxes < maxboxes do
        box_ = make_box()
        if not circleRectIntersect({x = 144, y = 80, radius = 30}, box_) then
            table.insert(boxes, box_)
        end
    end
    boxes[1] = {id = 1, x = 124, y = 140, width = 40, height = 100, color = {10, 10, 30}}
end
...
function lineIntersect(p1, q1, p2, q2)
    local o1 = get_orientation(p1, q1, p2)
    local o2 = get_orientation(p1, q1, q2)
    local o3 = get_orientation(p2, q2, p1)
    local o4 = get_orientation(p2, q2, q1)
    if o1 ~= o2 and o3 ~= o4 then
        return true
    elseif o1 == 0 and check_on_segment(p1, p2, q1) then
        return true
    elseif o2 == 0 and check_on_segment(p1, q2, q1) then
        return true
    elseif o3 == 0 and check_on_segment(p2, p1, q2) then
        return true
    elseif o4 == 0 and check_on_segment(p2, q1, q2) then
        return true
    end
    return false
end
...
function lineRectIntersect(a, b, rect)
    local r1 = {x = rect.x, y = rect.y}
    local r2 = {x = rect.x + rect.width, y = rect.y}
    local r3 = {x = rect.x + rect.width, y = rect.y + rect.height}
    local r4 = {x = rect.x, y = rect.y + rect.height}
    if lineIntersect(a, b, r1, r2) then
        return true
    elseif lineIntersect(a, b, r2, r3) then
        return true
    elseif lineIntersect(a, b, r3, r4) then
        return true
    elseif lineIntersect(a, b, r4, r1) then
        return true
    end
    return false
end
...
function circleRectIntersect(circle, rect)
    ...
    if dx > (rw / 2 + r) or dy > (rh / 2 + r) then
        return false
    end
...
function onTick()
    ...
    while diff > 0.01 and iterations < 10 do
        ...
        while node.parent do
            ...
            intersect = true
            for i = 1, #boxes do
                if lineRectIntersect(node.parent, node, boxes[i]) then
                    for ang = 0, pi, 0.05 do
                        pos = rotateAround(node.parent, node, ang)
                        if not lineRectIntersect(pos, node, boxes[i]) then
                            break
                        end
                        pos = rotateAround(node.parent, node, -ang)
                        if not lineRectIntersect(pos, node, boxes[i]) then
        ...
        while node.child do
            ...
            intersect = true
            for i = 1, #boxes do
                if lineRectIntersect(orig_pos, node, boxes[i]) then
                    for ang = 0, pi, 0.05 do
                        pos = rotateAround(orig_pos, node, ang)
                        if not lineRectIntersect(pos, node, boxes[i]) then
                            break
                        end
                        pos = rotateAround(orig_pos, node, -ang)
...
function drawTree(node)
    if node.child then
        ...
        for i = 1, #boxes do
            if lineRectIntersect(node.child, node, boxes[i]) then
                for ang = 0, pi / 2, 0.05 do
                    local pos = rotateAround(node.child, node, ang)
                    if not lineRectIntersect({x = pos.x, y = pos.y}, node,
                                             boxes[i]) then
                        screen.setColor(0, 255, 0)
                        screen.drawLine(pos.x, pos.y, node.x, node.y)
                        break
                    end
                    pos = rotateAround(node.child, node, -ang)
                    if not lineRectIntersect({x = pos.x, y = pos.y}, node,
                                             boxes[i]) then
                        screen.setColor(0, 255, 0)
                        screen.drawLine(pos.x, pos.y, node.x, node.y)
...
function onDraw()
    for b = 1, #boxes do
        screen.setColor(table.unpack(boxes[b].color))
        screen.drawRectF(boxes[b].x, boxes[b].y, boxes[b].width, boxes[b].height)
    end
    screen.setColor(255, 255, 255)
    drawTree(tree)
    -- print()
