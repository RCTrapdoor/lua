...
frames = {
    ...
    },
    {
        duration = 60,
        run = function(mul)
            drawRadians(w/2, h/2, 2, mul)
            screen.drawTextBox(w/2-r, y+7, 2*r, 5, "2 rad", 0, 0)
        end
    },
    {
        duration = 60,
        run = function(mul)
            drawRadians(w/2, h/2, 3, mul)
            screen.drawTextBox(w/2-r, y+7, 2*r, 5, "3 rad", 0, 0)
        end
    },
    {
        duration = 60,
        run = function(mul)
            drawRadians(w/2, h/2, math.pi, mul)
            screen.drawTextBox(w/2-r, y+7, 2*r, 5, "π rad", 0, 0)
        end
    },
    {
        duration = 60,
        run = function(mul)
            drawRadians(w/2, h/2, 2 * math.pi, mul)
            screen.drawTextBox(w/2-r, y+7, 2*r, 5, "2π rad", 0, 0)
        end
    },
}

function drawRadians(cx, cy, radians, mul)
    local segments = 64
    local radianSegments = radians * segments / (2 * math.pi)
    screen.setColor(50, 200, 50)
    drawSegment(cx, cy, 0, radians * mul, 20)
    screen.setColor(10, 50, 10)
    screen.drawLine(cx, cy, cx + r * math.cos(radians * mul), cy - r * math.sin(radians * mul))
    drawCurve(cx + 20, cy, radianSegments * mul, 20, radians * mul)
end
