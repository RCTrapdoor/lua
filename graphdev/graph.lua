...
channels = property.getNumber("Number of Channels") or 3
num_samples = property.getNumber("Number of Samples") or 600
...
function onTick()
	...
	isPressed = input.getBool(1) and (isPressed or 1) + 1 or false
	...
	if isPressed then
		if isPointInRectangle(inputX, inputY, 0, 151, 9, 9) then -- step back
			offset = math.min(offset + isPressed//5 + 1, #data[1].history - 232)
			offset1_button = ticks + 5

		elseif isPointInRectangle(inputX, inputY, 221, 151, 9, 9) then -- step forward
			offset = math.max(offset - isPressed//5 - 1, 0)
...
offset = 0
shades = {{100, 100, 100}, {140, 140, 140}, {55, 55, 55}, {0, 0, 0}}
function onDraw()
