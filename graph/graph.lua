...
do
    ...
    simulator:setProperty("Min/Max Value Notation", 1)
    simulator:setProperty("Current Value Notation", 1)
    simulator:setProperty("Sample every n-th tick", 1)
    simulator:setProperty("Number of Channels", 4)
    simulator:setProperty("Number of Samples", 600)
    simulator:setProperty("Label 1", "Channel 1")
	simulator:setProperty("Label 2", "Channel 2")
	simulator:setProperty("Label 3", "Channel 3")
	simulator:setProperty("Label 4", "Channel 4")
    ...
    function onLBSimulatorTick(simulator, ticks)
		...
		simulator:setInputNumber(10, math.sin(ticks / 100))
		simulator:setInputNumber(11, math.cos(ticks / 100))
		simulator:setInputNumber(12, math.sin(ticks / 100) * 0.5 + 0.5)
		simulator:setInputNumber(13, math.cos(ticks / 100) * 0.5 + 0.5)
	...
	if #data[1].history > 1 then
		...
		for k, v in ipairs(data) do
			...
			if #v.history > 0 then
				...
				screen.setColor(table.unpack(shades[3]))
				screen.drawLine(230, 16 * k - 1, 288, 16 * k - 1)
			end
		end
		--screen.setColor(table.unpack(shades[3]))
